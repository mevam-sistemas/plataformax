...
function mensagem(p:any){
  ...
  const abertura=p.produto==='360social' && p.papel==='pessoa_atendida'
    ? `Hoje celebramos a sua vida, ${nome}! É uma alegria caminhar perto de você e reconhecer o valor único da sua história.`
    : p.produto==='360social'
...
Deno.serve(async(req)=>{
  ...
  for(const p of data||[]){
    const assunto=p.produto==='360social'?`${primeiroNome(p.nome)}, hoje celebramos a sua vida!`:`Feliz aniversário, ${primeiroNome(p.nome)}!`;
    const r=await fetch('https://api.brevo.com/v3/smtp/email',{method:'POST',headers:{'api-key':Deno.env.get('BREVO_API_KEY')!,'content-type':'application/json'},body:JSON.stringify({sender:{name:p.produto==='360social'?'360social':'MODOX',email:'contato@arborlabs.com.br'},to:[{email:p.email,name:p.nome}],subject:assunto,htmlContent:mensagem(p)})});
